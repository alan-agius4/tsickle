...
class ExternsWriter extends ClosureRewriter {
  ...
  private writeExternsNamespaceExportDeclaration(
    ...
    // unconditionally exposing the global namespace, and then producing shim files that match
    // up a fake goog.module() with these global symbols.
    ...
    this.emit(`const ${exportName} = {};\n`);
    const exports = this.typeChecker.getExportsOfModule(sym);
    const exportedNamespace = [exportName];
    for (const exp of exports) {
      this.emit('/** @const @suppress {duplicate} */\n');
      const fqn = this.typeChecker.getFullyQualifiedName(exp);
      if (fqn.match(/^['"]/)) {
        // TODO(martinprobst): handle symbols declared inside of external
        // modules (with quotes) and make them accessible to Closure code. This
        // is particularly tricky as we'll need to track down if the augmented
        // external module was exported as a namespace, and if so, export onto
        // that namespace. If not, nothing should be emitted. For the time
        // being, this just emits a symbol that can be referenced, but has no
        // type information.
        this.writeExternsVariable(exp.name, exportedNamespace);
      } else {
